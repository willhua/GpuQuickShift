...
inline
float
distance(float const * I,         //计算五维的L2距离
         int N1, int N2, int K,
         int i1, int i2,
         int j1, int j2) 
{
...
void quickshift(image_t im, float sigma, float tau, float * map, float * gaps, float * E)
{
  ...
  R = (int) ceil (3 * sigma) ;  //计算概率时的窗口半径
  tR = (int) ceil (tau) ;       //在领域查找更大密度点时的半径
    ...
    for (i1 = 0 ; i1 < N1 ; ++ i1) {
      ...
      for (j2 = j2min ; j2 <= j2max ; ++ j2) {
        for (j1 = j1min ; j1 <= j1max ; ++ j1) {
          float Dij = distance(I,N1,N2,K, i1,i2, j1,j2) ;       //五维的L2距离，但这里面没有对位置和rgb做区分处理
    ...
    */
    ...
    for (i2 = 0 ; i2 < N2 ; ++i2) {
      for (i1 = 0 ; i1 < N1 ; ++i1) {
        ...
        //在半径为tR的窗口内，查找密度大于中心点，且距离最近的点，这个点被记录在map中，其距离被记录在gaps中
        //由于查找有窗口范围限制，所以，gaps为INF的点只能说时在局部最大概率，就和模点一样，不能说是全局概率最大
        float E0 = E [i1 + N1 * i2] ;
        float d_best = INF ;
        float j1_best = i1   ;
        float j2_best = i2   ; 
        ...
        map [i1 + N1 * i2] = j1_best + N1 * j2_best ; /* + 1 ; */
        if (map[i1 + N1 * i2] != i1 + N1 * i2)  //如果找到了概率更大的点
          gaps[i1 + N1 * i2] = sqrt(d_best) ;
        else
          gaps[i1 + N1 * i2] = d_best; /* inf */  //否则这个点就是局部概率最大的点，那么他的gaps为INF
      }
    }  
  }
